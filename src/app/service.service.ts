...
export class ServiceService {
  ...
  public validateEmail(username: any, password: any) {
    return this.http.get("http://localhost:2900/login/validate/" + username + "/" + password, { responseType: 'text' });
  }
  ...
  public AddAccount(cus) {
    return this.http.post("http://localhost:2001/AccountManagement/create", cus, { responseType: 'text' });
  }
  public ShowAccountDetails(accountId: string){
    return this.http.get("http://localhost:2001/AccountManagement/find/" + accountId, { responseType: 'json' })
  }
  public UpdateAccount(accountId: string, customerName: string, contactNumber: string, address: string) {
    ...
    return this.http.put("http://localhost:2001/AccountManagement/update/" + accountId + "/" + customerName + "/" + contactNumber + "/" + address, { responseType: 'text' });
  }
  ...
  updatePassbook(accountId:string){
    return this.http.get("http://localhost:2003/passbook/updatePassbook/"+accountId ,{responseType: 'json'});
  }
  accountSummary(accountId:string,StartDate:Date,EndDate:Date){
    return this.http.get("http://localhost:2003/passbook/accountSummary/"+accountId+"/"+StartDate+"/"+EndDate ,{responseType: 'json'});
  }
